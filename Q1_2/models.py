...
class DecisionTreeClassifier_KatzArbili(DecisionTreeClassifier):
    ...
    def predict_proba_rnd(self,x,in_alpha, to_print):
        try:
            ...
            for ind,x_row in enumerate(x.values):
                if (to_print):
                    print('x_row',x_row)
                ret_val.append(self.predict_proba_one(x_row,in_alpha,node_feature_id,node_use_random_arr[ind],to_print))
    ...
    probability of alpha decided to use the opposite branch of the node).
        ...
        Returns:
            predict (np.array): The calcualted prediction for each sample
    '''
    def predict_proba(self,x,in_alpha,in_n,to_print=False):
        try:
            ret_val = Parallel(n_jobs=48)(delayed(self.predict_proba_rnd)(x,in_alpha,to_print) for ind in range(in_n))
...
class DecisionTreeRegressor_KatzArbili(DecisionTreeRegressor):
    ...
    def predict(self,x,in_alpha,in_n,to_print=False):
        try:
            ret_val = Parallel(n_jobs=48)(delayed(self.predict_rnd)(x,in_alpha,to_print) for ind in range(in_n))
            return np.average(np.array(ret_val),axis=0)
        except Exception as e:
